...
function getCurrentWorkouts() {
  let curr_exercises = localStorage.getItem("exercise");
...
function modifyCurrentWorkouts(exercise) {
  ...
  localStorage.setItem("exercise", curr_exercises);
}
...
function saveExercise(exerciseId) {
  ...
  if (updatedExercise.weight <= 0 || updatedExercise.reps <= 0) {
    alert("Please Enter in Repititions or Weight Greater Than 0");
  } else {
